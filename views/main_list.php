...
$sql = "SELECT `url` FROM category WHERE `url` = '$parent'";
$result = mysqli_query($conn, $sql);
$rowcount = mysqli_num_rows($result);
?>
<?php if ($rowcount != 0) { //Biến parent có kết quả khác 0 
?>
    <main class="default-page-width">
        <section class="hot-banner">
            <div class="hot-banner-heading justify-content">
                <?php
                $sql = "SELECT * FROM category WHERE `url` = '$parent' LIMIT 0,1";
                $dm = executeResult($sql);
                $id_category = $dm[0]['id_category'];
                ?>
                <h1>
                    <a href="list.php?parent=<?php echo '' . $dm[0]['url'] . '' ?>" title="<?php echo '' . $dm[0]['metaTitle'] . '' ?>">
                ...
                <ul class="list-small">
                    ...
                    ?>
                    <?php foreach ($category_child as $dms) { ?>
                        <li>
                            <a href="list-small.php?parent=<?php echo '' . $dm[0]['url'] . '' ?>&child=<?php echo '' . $dms['url'] . '' ?>" title="<?php echo '' . $dms['metaTitle'] . '' ?>">
                                <?php echo '' . $dms['name'] . '' ?>
                            </a>
                        </li>
                    <?php } ?>
            ...
            <div class="hot-banner-main">
                <div class="hot-banner-main-x">
                    <div class="hot-banner-main-left">
                        <?php
                        $sql = "SELECT * FROM posts WHERE id_category = $id_category ORDER BY created_at DESC LIMIT 0,1";
                        ...
                        <?php
                        $sql = "SELECT * FROM posts WHERE id_category = $id_category AND id_posts != $id_posts1 ORDER BY created_at DESC LIMIT 0,3";
                        $posts = executeResult($sql);
        ...
        <section class="hot-sub">
            ...
            <div class="hot-sub-banner">
                ...
                <?php
                $sql = "SELECT id_posts,id_tag FROM posts_tag WHERE id_posts = $id_posts1 LIMIT 0,1";
                ...
                <div class="card-time">
                    <span><?php echo '' . date_format_vn($posts[0]['created_at']) . '' ?></span>
                </div>
            </div>
            <?php
            $sql = "SELECT * FROM posts WHERE id_category = $id_category 
        ...
        <section class="list-justify-content">
            <div class="tag-main">
                <div class="tag-main-left" id="load_data_list">
                    <?php
                    $rowperpage = 7;
                    $sql = "SELECT count(*) AS allcount FROM posts WHERE position = 0 AND id_category = $id_category 
                    ...
                    $sql = "SELECT * FROM posts WHERE id_category = $id_category AND position = 0
                        AND id_posts NOT IN ($id_posts1,$id_posts2,$id_posts3,$id_posts4,$id_posts5,$id_posts6,$id_posts7) 
                    ...
                    <button class="btn-list btn-parent">
                        <span>Xem Thêm</span>
                    </button>
                    ...
                    data-category="<?php echo '' . $id_category . '' ?>"
                    data-posts1="<?php echo '' . $id_posts1 . '' ?>" 
                    data-posts2="<?php echo '' . $id_posts2 . '' ?>" 
                    data-posts3="<?php echo '' . $id_posts3 . '' ?>" 
                    data-posts4="<?php echo '' . $id_posts4 . '' ?>" 
                    data-posts5="<?php echo '' . $id_posts5 . '' ?>" 
                    data-posts6="<?php echo '' . $id_posts6 . '' ?>" 
                    data-posts7="<?php echo '' . $id_posts7 . '' ?>">
                    <?php }  ?>
                </div>
                <div class="tag-main-right-ads">
                    <div class="sticky">
                        <?php
                        $sql = "SELECT * FROM posts WHERE id_category = $id_category  ORDER BY RAND() LIMIT 1";
                        $posts = executeResult($sql);
                        ?>
